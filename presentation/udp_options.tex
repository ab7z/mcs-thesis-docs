...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{Definierte Option-Typen:}

\begin{itemize}
    \item \textbf{OCS} -- Options Checksum \\
    {\small Sichert Middlebox-Traversal}

    \item \textbf{FRAG} -- Fragmentation \\
    {\small UDP-Level Fragmentierung, unabhängig von IP}

    \item \textbf{TIME} -- Timestamps \\
    {\small RTT-Messung und Rate Limiting}

    \item \textbf{AUTH} -- Authentication \\
    {\small Source-Validierung und Integritätsschutz}
\end{itemize}

\vspace{0.5em}
{\small Zusätzlich: EOL (End of List), NOP (Alignment)}
\end{column}
\begin{column}{0.4\textwidth}
\centering
