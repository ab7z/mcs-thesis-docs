...
\title{UDP Transport Options}
\subtitle{Implementierung und Analyse von RFC 9868 in Rust}
...
\begin{itemize}
    ...
    RFC 9868 definiert dafür die Option Checksum (OCS).
\end{itemize}
...
\vspace{0.5em}
{\small Umgesetzt: OCS, EOL, NOP, APC, FRAG, MDS, MRDS, REQ, RES \quad (TIME, AUTH: nicht im Umfang)}
\end{column}
\begin{column}{0.4\textwidth}
\centering
...
\begin{tikzpicture}[scale=0.75]
    ...
    \node[below] at (5.0,0) {\footnotesize 0 bis 255 Bytes};
\end{tikzpicture}
...
\textbf{In dieser Arbeit umgesetzt:}
\begin{itemize}
    \item Integritätsschutz der Surplus Area (OCS)
    \item UDP-Level Fragmentierung statt IP (FRAG)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Vom RFC vorgesehen, nicht im Umfang:}
\begin{itemize}
    \item Authentifizierung (AUTH)
    \item Path MTU Discovery (RFC 9869)
    \item RTT-Messung mit Timestamps (TIME)
\end{itemize}
\end{column}
\end{columns}
...
\begin{ganttchart}[
    ...
    \ganttbar{Implementierung (Rust)}{2}{4} \\
    \ganttbar{Netzpfad-Erprobung}{4}{5} \\
