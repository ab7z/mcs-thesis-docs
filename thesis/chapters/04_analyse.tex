...
\nolinkurl{thesis/daten/konformitaet-kategorien.csv} liegt im Repository dieser Arbeit,
...
(\secref{sec:zielsetzung}). Ausgewertet wird je Pfad, Richtung und Messfenster ein kontrolliertes
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}L{5.4cm}Y@{}}
    ...
    macOS-Messpunkt \texttt{en0} & zusätzlicher Beobachtungspunkt am Zugangsnetz;
...
(\secref{sec:pruefkette}). Auch \texttt{eval-check.py}, das bei den lokalen Referenzläufen und in den
Kampagnen eingesetzt wurde, prüft OCS selbst.
