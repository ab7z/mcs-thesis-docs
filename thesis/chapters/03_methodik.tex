...
Programmierung, formale Modellierung und Messkampagnen. Der Einsatz von KI-Werkzeugen wird deshalb nicht
beiläufig erwähnt, sondern vorab offengelegt und mit Prüfregeln versehen. Zur Einordnung dienen neun
...
\begin{itemize}
  ...
  \item \textbf{FunSearch:} Ein Sprachmodell erzeugte ausführbare Programmkandidaten, die ein
    ausführbarer Auswerter einzeln bewertete; das Ergebnis gilt nur für die kodierte Bewertungsfunktion
  ...
  \item \textbf{Zeta-Schranke:} Ein Modell lieferte ein mathematisches Ergebnis samt gemeinsamer
    Lean-Formalisierung; zwei Anthropic-Mathematiker und zwei externe Fachleute prüften es, ein
    dokumentierter Blindvergleich fand nicht statt \cite{anthropic2026zeta}.
...
Arbeitsteilung zwischen Autor und Werkzeugen steht fest, bevor Ergebnisse bewertet werden. Beides legen die
...
Die Versionsgeschichte schreibt diese Vorgaben dem Autor zu: Der erste Commit des Repositorys legte die
Roadmap und die Schrittdateien an, der zweite am selben Tag den Anforderungskatalog sowie die Architektur-
und Formatreferenz. Die Roadmap bezeichnet sich als Masterplan, der Schritt für Schritt mit einem Menschen
in der Schleife ausgeführt wird: ein geprüfter Commit je Schritt. Der Autor legt damit Plan, Maßstab und
Abschlusskriterien fest und gibt jedes Ergebnis frei; die Werkzeuge arbeiten innerhalb dieser Vorgaben.
Zwei Grenzen bleiben: Die Sammelcommits verschmelzen die innere Reihenfolge eines Schritts, und die
Commit-Historie trennt den Beitrag von Autor und Werkzeug je Änderung nicht auf.

Denselben Aufbau hat der Plan. Jeder Hauptschritt bekommt eine eigene Datei nach festem Schema mit Ziel,
...
Spezifikation vor der Umsetzung und den Beweis danach; ob jeder Schritt diese Reihenfolge einhielt, ist
...
Die Antworten der Tabelle sind Verfahrensregeln, und Verfahrensregeln führen Menschen und Modelle aus.
Ohne weiteres Zutun urteilen nur die ausführbaren Prüfungen und der Beweisprüfer; Pfadbefunde benötigen
zusätzlich das Sendemanifest und die dokumentierte Auswertung. Wie diese Prüfungen für FF1 aufgebaut sind,
...
Dieser Abschnitt beschreibt ausschließlich die Prüfung der Entwicklung, also die Nachweisseite von FF1.
...
\tabref{tab:operationalisierung} fest, das Pfad- und Fehlermodell folgt in \secref{sec:ff2-pfadmodell},
die Belegregeln und die Auswertung der Messläufe in \secref{sec:testkonzept}. Der eigene Mitschnittprüfer
...
Integrationstests. Drei weitere Prüfmittel tragen besondere Aufgaben. Ein Property-Test formuliert eine
Eigenschaft, die für alle Eingaben einer Klasse gelten soll, und prüft sie an vielen maschinell erzeugten
Fällen statt an ausgewählten Beispielen \cite{proptest}. Ein Fuzz-Ziel ist eine Funktion, die eine
...
\cite{cargo-fuzz,libfuzzer}. Fuzzing findet Fehler, aber keine Fehlerfreiheit; es läuft als lokale
Pflichtstufe mit Zeitschranke und nicht in der kontinuierlichen Integration, ein unauffälliges Ergebnis
bedeutet also nur, dass innerhalb der Zeitschranke kein Fund auftrat. Der dokumentierte Sollprozess
verlangt, gefundene Gegenbeispiele zu minimieren und als Regressionstests einzuchecken. Lean ist ein
Beweisassistent: Regeln werden als Modell formuliert, Aussagen darüber als Theoreme bewiesen, und ein
...
Rust-Quelltext; den gebauten Umfang beziffert \secref{sec:pruefarchitektur}.
...
Längenkonstanten. Ein gemeinsamer Entwurfsfehler in diesen Annahmen kann deshalb jeden Test
...
projekteigenen Formatdokumentation. Die Unabhängigkeit gilt für den Prüfcode, nicht für den Entwurf.
Dasselbe gilt für die drei handgerechneten OCS-Referenzvektoren der Unit-Tests. Sie sind nicht
palindromisch und wurden getrennt nachgerechnet, damit Bytevertauschungen auffallen.

\subsection{Prüfkette bis zur Freigabe}

Das lokale Gate \texttt{pre-pr.sh} fährt neun feste Bahnen: Formatierung, statische Analyse,
Dokumentationsbau, die Tests samt Property-Fällen, die Prüfung des eigenen Auswerters, das Lean-Tor sowie
drei Bahnen auf einem separaten Linux-Zielsystem, das die Cross-Kompilierung, die Raw-Socket-Integration
mit Root-Rechten und das tatsächlich gesendete Paketbild prüft. Dazu kommt je Fuzz-Ziel eine eigene Bahn;
zusammen ergeben sich die 18 Prüfläufe aus \figref{fig:stepzyklus}. Die kontinuierliche Integration (CI)
...
Das Projekt setzt kein systematisches Mutation-Testing am Rust-Quelltext ein; in einem dokumentierten
Einzelfall wurde ein früherer Quelltextfehler gezielt wieder eingeführt, und drei Property-Tests sowie ein
Fuzz-Ziel erkannten diese Mutation. Gezielte Veränderungen an Paketmitschnitten prüfen nur, ob der
jeweilige Auswerter bekannte Manipulationen erkennt. Ob eine Prüfung tatsächlich stattfand, entscheidet
allein ihre Protokollierung; deren Grenzen behandelt der folgende Abschnitt.
...
Zum Abschluss dieses Kapitels bleiben offene Grenzen. Die drei handgerechneten OCS-Referenzvektoren sind
bislang nur projektintern gegengerechnet; eine externe Bestätigung steht aus. Die zufallsgespeisten
Prüfungen sind zudem nur begrenzt wiederholbar: Property- und Fuzz-Läufe sind nicht auf feste Startwerte
festgelegt, und der Fuzz-Korpus ist unversioniert; reproduzierbar sind die versionierten Beispieleingaben,
die Protokolle und die als Regressionstests gesicherten Gegenbeispiele, nicht der einzelne Lauf. Die
Arbeitsteilung schließlich ist durch die versionierten Vorgabedokumente belegt; die Commit-Historie trennt
den Beitrag von Autor und Werkzeug je Änderung nicht auf.
